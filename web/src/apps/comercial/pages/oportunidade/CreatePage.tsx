...
type OpportunityOut = OpportunityIn & {
  id: string;
...
export function OportunidadeCreatePage() {
  const nav = useNavigate();
  const { user } = useAuth();

  const accountDatalistId = "opportunity-create-account-options";
  ...
  const canSave = useMemo(
    () => !!name.trim() && !!ownerId.trim() && (!opportunityStages.length || !!stage.trim()) && !saving,
    [name, ownerId, opportunityStages.length, stage, saving],
  );
    ...
    try {
      const [sessionsData, defsData] = await Promise.all([
        apiFetch<CustomFieldSession[]>(
          `/crm/provisioning/field-sessions?entity_type=${encodeURIComponent("opportunity")}`,
          { signal } as any,
        ).catch(() => [] as CustomFieldSession[]),
        apiFetch<CustomFieldDef[]>(
          `/crm/provisioning/fields?entity_type=${encodeURIComponent("opportunity")}`,
          { signal } as any,
        ),
      ]);
  ...
  useEffect(() => {
    if (!opportunityStages.length) return;
    if (opportunityStages.some((s) => s.value === stage)) return;
    setStage(opportunityStages[0].value);
    ...
    (async () => {
      try {
        await Promise.all([
          ...
          (async () => {
            const [us, accs, stages] = await Promise.all([
              ...
              apiFetch<OpportunityStageOption[]>("/crm/opportunity-stages", { signal: ac.signal } as any).catch(
                () => [] as OpportunityStageOption[],
              ),
            ]);
            setUsers(us.filter((u) => u.is_active !== false));
            setAccounts(accs);
            setOpportunityStages(stages.filter((s) => s.is_active !== false));
          })(),
        ]);
      } finally {
        setLoading(false);
  ...
  async function save() {
    ...
    try {
      ...
      const created = await apiFetch<OpportunityOut>("/crm/opportunities", {
        method: "POST",
        csrf: true,
        body: payload,
      });
      nav(`/apps/comercial/oportunidades/${created.id}`);
    } catch (e) {
      setErr(extractApiErrorMessage(e));
    } finally {
      setSaving(false);
    ...
    <div className="h-full min-h-0">
      <section className="panel flex h-full min-h-0 flex-col overflow-hidden border border-[rgb(var(--border))] !rounded-none">
        <header className="shrink-0 flex flex-col gap-3 border-b border-[rgb(var(--border))] bg-[rgb(var(--panel-2))] px-4 py-3 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-base font-semibold">Nova Oportunidade</div>
          ...
          <div className="space-y-4 p-4">
            ...
            <section className="overflow-hidden border-t border-[rgb(var(--border))]">
              ...
              <div className="bg-[rgb(var(--panel))]">
                <div className="grid grid-cols-1 border-t border-[rgb(var(--border))] md:grid-cols-2">
                  ...
                  <div className="border-b border-[rgb(var(--border))] p-3">
                    <label className="text-sm text-[rgb(var(--muted))]">Etapa</label>
                    <select
                      className="input mt-1 w-full"
                      value={stage}
                    ...
                    >
                      {!opportunityStages.length && (
                        <option value="">Cadastre em Configurações &gt; Stages da Oportunidade</option>
                      )}
                    ...
                    <div className="mt-1 text-xs text-[rgb(var(--muted))]">
                      Os valores são definidos em Configurações → Apps → Comercial → Oportunidade → Stages da
                      Oportunidade.
                    </div>
                  </div>

                  <div className="border-b border-[rgb(var(--border))] p-3">
                    <label className="text-sm text-[rgb(var(--muted))]">Valor</label>
                    <MoneyInput className="input mt-1 w-full" value={amount} onChange={setAmount} min={0} disabled={loading || saving} />
                  </div>

                  <div className="border-b border-[rgb(var(--border))] p-3">
                    <label className="text-sm text-[rgb(var(--muted))]">Data de fechamento</label>
                    <input
                      className="input mt-1 w-full"
                      type="date"
                      value={closeDate}
            ...
            <div className="border-t border-[rgb(var(--border))]">
              <CustomFieldsBySession
                ...
                mode="create"
                emptyLabel="Nenhum campo customizado ativo para Oportunidade."
