...
def test_rls_blocks_cross_tenant_insert() -> None:
    """
    Integration test that proves RLS blocks cross-tenant INSERTs.
    ...
    # Run alembic migrations as admin (ConfigParser needs %% escaping for URL-encoded passwords)
    ...
    with engine.begin() as conn:
        # Ensure business_units exist for FK constraints so FK doesn't mask RLS
        conn.execute(
            text(
                """
        ...
        # Tenant context to BU1
        conn.execute(text("SELECT set_config('app.tenant_id', :tid, true)"), {"tid": bu1})

        # Cross-tenant insert BU2 must fail due to WITH CHECK
